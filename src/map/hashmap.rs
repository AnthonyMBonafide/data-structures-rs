...
impl<K, V> Map<K, V> for MyHashmap<K, V>
where
    K: PartialEq + Clone + Hash,
    V: Clone,
{
    fn insert(&mut self, key: K, value: V) {
        ...
        self.hash_elements[bucket_index].insert(key, value);
    }

    fn get(&self, key: &K) -> Option<&V> {
        let hash = (self.hasher)(key);
        let bucket_index = hash % self.hash_elements.len();
        self.hash_elements[bucket_index].find(key)
    }
    fn clear(&mut self) {
    ...
    fn remove(&mut self, key: &K) {
        let hash = (self.hasher)(key);
        let bucket_index = hash % self.hash_elements.len();
        self.hash_elements[bucket_index].remove(key);
    }
}
